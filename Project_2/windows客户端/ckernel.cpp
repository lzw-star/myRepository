...
///
/// [net]  组名
/// ip="192.168.31.51"   -- key=value
///
/// .exe 同级的目录下面添加 config.ini
#include<QSettings>
...
//    m_pWeChatDlg->show();
    ...
    for( int i = 0 ; i < 2 ; i++){
        ...
        m_pAVClient[i]->OpenNet( /*_DEF_SERVERIP*/m_serverip , _DEF_PORT );
    ...
    m_pSendVideoWorker
            = QSharedPointer<SendVideoWorker>( new SendVideoWorker);
...
void Ckernel::slot_destroy()
{
    ...
    {
        m_pRoomdialog->hide();
        delete m_pRoomdialog; m_pRoomdialog = NULL;
    }
    if( m_pClient )
    {
        m_pClient->CloseNet();
...
//    rq->m_szUser;
    //创建用户对应的控件
    UserShow * user = new UserShow;
    user->slot_setInfo( rq->m_UserID
                        , QString::fromStdString(rq->m_szUser) );
